	...
	t.Run("space is flagged as community", func(t *testing.T) {
		...
		require.NoError(t,
			waitpoll.Poll(hostAwait.RetryInterval, hostAwait.Timeout, func() (bool, error) {
				...
				if err != nil {
					t.Logf("error building LabelRequirement for SpaceBinding's MasterUserRecord key: %v", err)
					return false, err
				}

				sr, err := labels.NewRequirement(toolchainv1alpha1.SpaceBindingSpaceLabelKey, selection.Equals, []string{space.Name})
				if err != nil {
					t.Logf("error building LabelRequirement for SpaceBinding's Space key: %v", err)
					return false, err
				}
				...
				if err := hostAwait.Client.List(context.TODO(), sbs, opts); err != nil {
					t.Logf("error retrieving list of SpaceBinding with labels selector for MUR and Space: %v", err)
					return false, err
				}

				t.Logf("retrieved %v space bindings", len(sbs.Items))
